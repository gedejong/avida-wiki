...
- **Next candidate**: Extract deterministic `cResourceCount` write-path selection policy (geometry-based global-vs-spatial branching in `Set`/`ModifyCell`) behind additive Rust FFI while preserving C++ mutation ordering and ownership.
