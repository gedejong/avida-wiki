...
- **Next candidate**: Extract deterministic `cResourceCount` read-path selection policy (`IsSpatialResource`/geometry-based branch choosing between global array and spatial cell accessors) behind additive Rust FFI while preserving C++ ownership and query ordering.
